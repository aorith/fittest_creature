...
ONLY_RECORD_BREEDS = True  # Switches how we choose the creatures that breed, with this
...
class Creature(pg.sprite.Sprite):
    ...
    def breed(self):
        ...
        age_chance = translate(self.age, 50, 1000, 0.01, 0.5)
        if (self.age > BREEDING_AGE and random() < age_chance) or ONLY_RECORD_BREEDS:
            now = pg.time.get_ticks()
            # check if it has bred recently, if so, the creature can't breed again yet
            if (now - self.last_bred_time > BREEDING_WAIT) or ONLY_RECORD_BREEDS:
...
class Game:
    ...
    def spawn_creatures(self):
        if not ONLY_RECORD_BREEDS:
            ...
            # only record creature can breed (if exists)
            if not self.all_creatures:
                if self.record is None:
                    # We don't have a record creature yet, spawn first gen
                    ...
                    # We do have a record, he will breed all the creatures
                    while len(self.all_creatures) < TOTAL_CREATURES:
                        newpos = vec(randint(0, WIN_WIDTH),
                                     randint(0, WIN_HEIGHT))
                        # force to breed
                        dna = None
                        while dna is None:
                            dna = self.record.breed()

                        if valid_pos(newpos, self.poison_group):
                            child = Creature(newpos, dna)
                            self.all_creatures.add(child)
                            self.all_sprites.add(child)
                            self.record.childs += 1
                            child.gen += 1 + self.record.gen
